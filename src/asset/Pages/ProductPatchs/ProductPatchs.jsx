...
export default function SizeDemo() {
    ...
    const copyProductId = (product) => {
        navigator.clipboard.writeText(product._id)
            .then(() =>
                toast.current.show({
                    severity: 'success',
                    summary: 'Đã sao chép',
                    detail: `ID sản phẩm: ${product._id}`,
                    life: 3000
                })
            )
            .catch(() => {
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <i
                    className="pi pi-trash hover:bg-red-400"
    ...
    const actionCopyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <i
                    className="pi pi-copy hover:bg-bluegray-100"
                    onClick={() => copyProductId(rowData)}
                ></i>
            </React.Fragment>
        );
    };
    ...
    const rowExpansionTemplate = (data) => {
        ...
        var url = `${process.env.REACT_APP_API_URL}/api/v1/processors/upload/${product._id}`;
        return (
            <>
                <TabView>
